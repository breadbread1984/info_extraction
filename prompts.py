...
def exists_electrolyte_template(tokenizer):
  class Result(BaseModel):
    present: str = Field(description = "a string between 'present' and 'not present' will represents whether the ratio of elements is given in the context.")
    detail: Union[Dict[str,str], str] = Field(description = "if the ratio of elements is present in the context, return a dictionary whose keys are elements and values are their proportion. if the ratio of elements is present outside the context, return where the ratio is given in string format.")
  parser = JsonOutputParser(pydantic_object = Result)
  instructions = parser.get_format_instructions()
  system_message = """The following text is about how an electrolyte is produced. Please extract the elements and their proportions in the electrolyte introduced in the context if the information is available. The ratio of elements can be given by the reading of ICP of the electrolyte or by the chemical formula of the electrolyte. Please extract where the ratio of the elements is given outside the context if the information is not available in this context. The context should refer the reader to a specific table for the ratio of the elements.

Here are two examples of context which gives information about the ratio of elements.
...
Here are an example of context which refers reader to a table
