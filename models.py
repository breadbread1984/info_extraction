...
def Llama3_FA2(locally = False):
  ...
  class LLama3FA2(LLM):
    ...
    def __init__(self,):
      ...
      self.model = AutoModelForCausalLM.from_pretrained('meta-llama/Meta-Llama-3.1-8B-Instruct', trust_remote_code = True)
...
def Qwen2_FA2(locally = False):
  ...
  class Qwen2FA2(LLM):
    ...
    def __init__(self,):
      ...
      self.model = AutoModelForCausalLM.from_pretrained('Qwen/Qwen2-7B-Instruct', trust_remote_code = True)
      self.model = self.model.to(device('cuda'))
      self.model.eval()
    def _call(self, prompt, stop = None, run_manager = None, **kwargs):
      logits_processor = LogitsProcessorList()
